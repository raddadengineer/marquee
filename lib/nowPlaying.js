...
// Tautulli is the only real dependency this whole module has (Plex itself is
// never optional — the sign-in flow requires it) — so skip cleanly rather
// than hitting a guaranteed-to-fail request every 10s if it's not
// configured. A single no-op refresh() call is cheap enough that gating
// just this one spot (rather than also guarding the setInterval in start())
// is sufficient.
