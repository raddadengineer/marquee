...
// Rewrites the KEY=value lines named in `updates` ({KEY: newValue}), leaving
// comments, blank lines, ordering, and every other key untouched. Any update
// key that isn't already a line in the file gets appended as a new one —
// happens when a service registry field (routes/settings.js) is set for the
// first time on a deployment whose .env predates that field being added.
