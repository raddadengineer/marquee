...
const DEFAULT_PRIVACY_CONFIG = {
  ...
  streamTechnical: 'full_technical',   // 'full_technical' | 'hide_network_ip' | 'hide_device_info' | 'hide_all_transcode'
...
function sanitizeSession(session, requestingUser, config = getPrivacyConfigFromEnv()) {
  ...
  // 3. Technical details
  if (config.streamTechnical === 'hide_network_ip') {
    delete s.ipAddress;
    delete s.location;
  } else if (config.streamTechnical === 'hide_device_info') {
    delete s.player;
    delete s.platform;
    delete s.device;
  } else if (config.streamTechnical === 'hide_all_transcode') {
    delete s.ipAddress;
    delete s.location;
    delete s.player;
    delete s.platform;
